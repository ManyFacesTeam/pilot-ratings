...
```{r}
# model data are not shared on github but are available via UK Data Service ReShare
...
#### Model data
**recoding of gender, ethnicity, height, and weight**
```{r}
ethnicity_recode <- read_csv("data/eth_recode.csv", na = "", show_col_types = FALSE)
gender_recode <- read_csv("data/gender_recode.csv", na = "", show_col_types = FALSE)
```
...
#### Questionnaire data
**recoding of  ethnicity**
```{r}
quest_eth_recode <- read_csv("data/quest_eth_recode.csv", na = "", show_col_types = FALSE)
...
**Remove duplicate trials and incomplete data**
...
exclusions |> 
  summarise(
    ...
    `Failed more than 1 of 7 attention checks` = sum(!is.na(attn_checks_passed)),
    "Total exclusions" = n()
  ) |> 
  pivot_longer(
    cols = everything(),
...
data_quest |> 
  ...
  select(-try) |>
  write_csv("data/manyfaces_ratings_quest_cleaned.csv")

# Exclusions data
write_csv(exclusions, "data/exclusions.csv")

# Model data
write_csv(data_model, "ReShare_data/manyfaces_model_data_cleaned.csv")
```